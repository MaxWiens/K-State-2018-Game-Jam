...
function draw(self, xMod, yMod)
	if isActive then
		love.graphics.draw(self._tileSet.image, self._tileSet.quads[6], self._x + xMod, self._y + yMod)
	else
		love.graphics.draw(self._tileSet.image, self._tileSet.quads[3], self._x + xMod, self._y + yMod)
	end
end
